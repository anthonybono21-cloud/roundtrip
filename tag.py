...
def tags_for(cam):
    ...
    add(*cam.get('curated_tags', []))
    if isinstance(cam.get('quality_score'), int):
        add(f"quality-{cam['quality_score']}", f"score-{cam['quality_score']}")
...
def main():
    ...
    data['tag_notes'] = [
        ...
        'plus hand-written extras and each record\'s curated_tags. Preserve researched '
        'scene tags in curated_tags; derived tags come from tag.py.',
